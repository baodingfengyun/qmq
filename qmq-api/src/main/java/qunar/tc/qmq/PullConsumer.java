...
public interface PullConsumer extends BaseConsumer, AutoCloseable {
     ...
     * 设置false时，可靠消息不会自动ack，需要调用ack方法，非可靠不需要。 默认值是false。
     */
    ...
    /**
     * 拉取到size个消息后才返回。 如果producer发送的消息个数没有达到size，则pull方法会被阻塞住。
     * 如果当前拉取线程被Interrupted, 则返回已经拉取到的消息。 返回的消息个数可能不等于size。
     * <p>
     * 对于可靠消息处理完成后，必须调用Message的ack方法，然后再次拉取。 对于非可靠消息，无需调用ack方法。
     */
    List<Message> pull(int size);

    /**
     * 尝试在timeoutMillis内拉取size个消息。 返回的消息个数可能不等于size。 如果size <=
     * 0，会立即返回一个空的List<Message>。
     * <p>
     * 当broker上剩余的消息数小于size时，pull方法会阻塞至超时。 实际调用时间可能大于timeoutMillis。
     ...
     * 否则可能出现拉取到的消息没有被获取到。 timeoutMillis的设置参考
     * qunar.tc.qmq.PullConsumer#pull(int, long) 的注释
     * <p>
     * 对于可靠消息处理完成后，必须调用Message的ack方法，然后再次拉取。 对于非可靠消息，无需调用ack方法。
     */
